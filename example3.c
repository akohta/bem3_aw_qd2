...
void p_field_x(IMD *id,AQD2 *ad);
void p_field_y(IMD *id,AQD2 *ad);
void p_field_z(IMD *id,AQD2 *ad);
void output_field(char *pl,IMD *id,AQD2 *ad);
...
int main(int argc,char *argv[])
{
  ...
  dat_read_aqd2(argv[1],&ad); // read datafile 
  print_aqd2(&ad);       // print data 
...
void p_field_x(IMD *id,AQD2 *ad)
{
  ...
  for(i=0;i<id->m;i++){
    ...
    for(j=0;j<id->m;j++){
      ...
      pv_t_aqd2(&p,v,x,id->type,ad); // total field
